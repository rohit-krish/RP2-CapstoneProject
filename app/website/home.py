...
def home_func():
    ...
    names_df = pd.read_parquet(file_path)
    similarity_df = pd.read_parquet('df_similarity_bow.parquet')
    return render_template("index.html")

def recommend(id, limit=8):
    movie_index = names_df[names_df['id'] == int(id)].index[0]
    ...
    return [names_df.iloc[m[0]].id for m in movies_list]


def get_data(names_df):
